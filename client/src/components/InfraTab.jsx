...
function InfraTab() {
  ...
  const handleServerChange = (id) => {
    ...
    showToast(`${t('server_switched_to')}: ${srv?.name}`, 'info');
  };
  ...
  const formatBytes = (bytes) => {
    if (!bytes) return `0 ${t('gb')}`;
    return (bytes / 1e9).toFixed(2) + ` ${t('gb')}`;
  };

  const statCards = [
    { 
      label: isMainHub ? `${t('costs')} (Vultr)` : `${t('costs')} (Hetzner)`, 
      value: isMainHub ? ('$' + (status?.pending_charges ?? '0.00')) : (isAiNode ? '€19.90' : '€0.00'), 
      icon: <CreditCard size={18} />, 
      color: '#10b981', 
      sub: t('monthly_flat_rate') 
    },
    { label: t('system_availability'), value: stats?.uptime || '99.9%', icon: <Zap size={18} />, color: '#f59e0b', sub: t('all_nodes_ok') },
    { label: t('data_transfer'), value: bandwidth ? formatBytes(bandwidth.outgoing_bytes) : '---', icon: <Activity size={18} />, color: '#6366f1', sub: t('outgoing_traffic') }
  ];
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: '2rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="glass-card" style={{ padding: '1.5rem', borderRadius: '20px', flex: 1 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', paddingBottom: '1rem', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <h3 style={{ fontSize: '0.95rem', fontWeight: '800', letterSpacing: '0.05em' }}>{t('server_details').toUpperCase()}</h3>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(255,255,255,0.03)', padding: '0.4rem 0.8rem', borderRadius: '30px', border: '1px solid var(--card-border)' }}>
                 ...
                 <span style={{ fontSize: '0.7rem', fontWeight: '800', textTransform: 'uppercase', color: statusColor }}>{status?.power_status ? t(status.power_status.toLowerCase()) : t('inactive').toUpperCase()}</span>
              </div>
            </div>

            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                 <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', fontWeight: '700' }}>{t('ip_address').toUpperCase()}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                 <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', fontWeight: '700' }}>{t('system_region').toUpperCase()}</div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                 <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', fontWeight: '700' }}>{t('data_transfer').toUpperCase()}</div>
                 <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', fontSize: '0.85rem', fontWeight: '700' }}>
          ...
          <div className="glass-card" style={{ padding: '1.25rem', borderRadius: '20px' }}>
            <h3 style={{ fontSize: '0.85rem', fontWeight: '800', color: 'var(--text-secondary)', marginBottom: '1rem', letterSpacing: '0.05em' }}>{t('power_and_restart').toUpperCase()}</h3>
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              ...
              >
                <Play size={14} fill="currentColor" /> {t('start').toUpperCase()}
              </button>
              <button 
              ...
              >
                <RefreshCcw size={14} /> {t('restart').toUpperCase()}
              </button>
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          ...
          <div className="glass-card" style={{ padding: '1.25rem', borderRadius: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
              ...
              <h3 style={{ fontSize: '0.85rem', fontWeight: '800', color: 'var(--text-secondary)', letterSpacing: '0.05em' }}>{t('continuous_deployment_git').toUpperCase()}</h3>
            </div>
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              ...
              >
                <Github size={14} /> {t('pull').toUpperCase()}
              </button>
            </div>
          </div>
          ...
          <div className="glass-card" style={{ padding: '1.25rem', borderRadius: '20px', flex: 1, display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <h3 style={{ fontSize: '0.85rem', fontWeight: '800', color: 'var(--text-secondary)', letterSpacing: '0.05em' }}>{t('remote_ssh_terminal').toUpperCase()}</h3>
              </div>
            </div>
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <input 
          ...
          <div className="glass-card" style={{ padding: '1.25rem', borderRadius: '20px' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '1rem' }}>
              ...
              <h3 style={{ fontSize: '0.85rem', fontWeight: '800', color: 'var(--text-secondary)', letterSpacing: '0.05em' }}>{t('android_relay_apk').toUpperCase()}</h3>
            </div>
            ...
            {apkInfo?.available ? (
              <div style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.2)', borderRadius: '12px', padding: '0.75rem 1rem', marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  ...
                  <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)', marginTop: '0.2rem' }}>
                    {(apkInfo.size / 1024 / 1024).toFixed(1)} MB · {t('uploaded_at')} {new Date(apkInfo.uploadedAt).toLocaleDateString(lang === 'cz' ? 'cs-CZ' : 'en-US')}
                  </div>
                </div>
              ...
              <div style={{ background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.2)', borderRadius: '12px', padding: '0.75rem 1rem', marginBottom: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontSize: '0.75rem', fontWeight: '800', color: '#a5b4fc', display: 'flex', alignItems: 'center', gap: '0.4rem' }}><Smartphone size={13} /> {t('no_apk')}</div>
                  <div style={{ fontSize: '0.6rem', color: 'var(--text-secondary)', marginTop: '0.15rem' }}>{t('no_apk_desc')}</div>
                </div>
              </div>
            )}
            ...
            >
              {uploadProgress !== null ? (
                <>
                  <div style={{ fontSize: '0.72rem', color: '#a5b4fc', fontWeight: '800', marginBottom: '0.5rem' }}>{t('uploading')}... {uploadProgress}%</div>
                ...
                <>
                  ...
                  <div style={{ fontSize: '0.78rem', color: '#a5b4fc', fontWeight: '900' }}>{t('drop_to_upload_apk')}</div>
                </>
              ) : (
                <>
                  <Upload size={22} style={{ color: '#a5b4fc', marginBottom: '0.4rem', opacity: 0.7 }} />
                  <div style={{ fontSize: '0.78rem', color: '#a5b4fc', fontWeight: '800', marginBottom: '0.2rem' }}>{t('upload_new_apk')}</div>
                  <div style={{ fontSize: '0.65rem', color: 'var(--text-secondary)' }}>{t('version_auto_detected')}</div>
                </>
              )}
            </div>
