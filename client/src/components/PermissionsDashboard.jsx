    ...
    <div style={{ padding: isMobile ? '1rem' : '2rem', flex: 1, overflowY: 'auto' }} className="fade-in custom-scrollbar">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fill, minmax(380px, 1fr))', gap: isMobile ? '1rem' : '2rem' }}>
        {Object.entries(rolePermissions).map(([role, perms]) => {
          ...
          return (
            <div key={role} className="glass-card" style={{ padding: '2rem', height: 'fit-content', border: isAppOwner ? '1px solid rgba(251, 191, 36, 0.3)' : undefined }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  <div style={{ width: '40px', height: '40px', background: isAppOwner ? 'rgba(251, 191, 36, 0.15)' : 'rgba(59, 130, 246, 0.1)', borderRadius: '10px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {isAppOwner ? <Lock size={20} color="#fbbf24" /> : <Shield size={20} color="var(--accent-color)" />}
                  </div>
                  <h3 style={{ fontSize: '1.25rem', fontWeight: '800' }}>{role}</h3>
                </div>
                {isAppOwner ? (
                  <div className="status-badge-small" style={{ borderColor: '#fbbf24', color: '#fbbf24', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                    <Lock size={10} /> LOCKED
                  </div>
                ) : (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                {Object.entries(perms).map(([permKey, isEnabled]) => (
                  <div key={permKey} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '0.75rem', borderBottom: '1px solid rgba(255,255,255,0.03)' }}>
                    ...
                    {isAppOwner ? (
                      // App Owner permissions are locked and cannot be toggled
                      <div style={{
                        ...
                        <div style={{
                          width: '12px', height: '12px', background: isEnabled ? '#fbbf24' : 'rgba(255,255,255,0.3)', borderRadius: '50%',
                          position: 'absolute', top: '2px', left: isEnabled ? '18px' : '3px'
                        }}></div>
                      </div>
                    ) : (
                      <div 
