...
const MobileBottomNav = () => {
  ...
  // Relay APK builds set __APP_VARIANT__ = 'relay' at compile time (via vite define)
  ...
  // Relay APK: only Dashboard + Relay tab
  // Full app: Dashboard, Inbox, Calendar (filtered by permissions)
  const allTabs = isRelayApp
    ? [
        { id: 'dashboard', icon: DashboardIcon, label: t('dashboard') },
        { id: 'relay',     icon: ZapIcon,        label: t('relay') || 'Relay' },
      ]
    : [
        { id: 'dashboard', icon: DashboardIcon, label: t('dashboard') },
  ...
  const tabs = isRelayApp
    ? allTabs
    : allTabs.filter(tab => {
        if (activeOperator?.isAppOwner || activeOperator?.isAdmin) {
          if (tab.id === 'calendar') return false;
        }
